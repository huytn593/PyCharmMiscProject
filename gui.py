...
class MusicUploaderGUI:
    ...
    def setup_ui(self):
        ...
        # Title
        ttk.Label(main_frame, text="Music Track Uploader", font=('Helvetica', 16, 'bold')).grid(row=0, column=0, columnspan=2, pady=10)
        ...
        # Title
        ttk.Label(info_frame, text="Title:").grid(row=0, column=0, sticky=tk.W)
        ttk.Entry(info_frame, textvariable=self.title_var, width=40).grid(row=0, column=1, sticky=(tk.W, tk.E), padx=5)
        
        # Album
        ttk.Label(info_frame, text="Album:").grid(row=1, column=0, sticky=tk.W)
        ttk.Entry(info_frame, textvariable=self.album_var, width=40).grid(row=1, column=1, sticky=(tk.W, tk.E), padx=5)
        
        # Genres
        ttk.Label(info_frame, text="Genres:").grid(row=2, column=0, sticky=tk.W)
        ttk.Entry(info_frame, textvariable=self.genres_var, width=40).grid(row=2, column=1, sticky=(tk.W, tk.E), padx=5)
        ttk.Label(info_frame, text="(comma-separated)").grid(row=2, column=2, sticky=tk.W)
        
        # Public/Private
        ttk.Checkbutton(info_frame, text="Make Public", variable=self.is_public_var).grid(row=3, column=1, sticky=tk.W)
        
        # File Upload Section
        files_frame = ttk.LabelFrame(main_frame, text="File Upload", padding="10")
        files_frame.grid(row=2, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=5)
        
        # MP3 File
        mp3_frame = ttk.Frame(files_frame)
        mp3_frame.grid(row=0, column=0, sticky=(tk.W, tk.E), pady=5)
        ttk.Label(mp3_frame, text="MP3 File:").grid(row=0, column=0, sticky=tk.W)
        self.mp3_label = ttk.Label(mp3_frame, text="Drop MP3 file here or click to browse")
        self.mp3_label.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=5)
        ...
        cover_frame.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=5)
        ttk.Label(cover_frame, text="Cover Image:").grid(row=0, column=0, sticky=tk.W)
        self.cover_label = ttk.Label(cover_frame, text="Drop image file here or click to browse")
        self.cover_label.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=5)
        ...
        # Upload Button
        ttk.Button(main_frame, text="Upload Track", command=self.upload_track).grid(row=3, column=0, columnspan=2, pady=20)
    ...
    def upload_track(self):
        ...
        try:
            ...
            success = add_track(
                ...
                album=self.album_var.get() or None,
                genres=genres,
    ...
    def clear_form(self):
        ...
        self.mp3_path = None
        self.cover_path = None
        self.mp3_label.config(text="Drop MP3 file here or click to browse")
        self.cover_label.config(text="Drop image file here or click to browse")
